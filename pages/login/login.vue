<template>
	<view>
		...
		<view class="form">
			...
			<view class="btn mb-3" @tap="doLogin">登 录</view>
			<view class="btn" @tap="LoginAccount">普通登录</view>
			<view class="res">
		...
		<view class="oauth" v-if="isShowOauth">
			...
			<view class="list">
				...
				<button type="default" open-type="getPhoneNumber" @getphonenumber="loginPhoneNumber">快速登录</button>
				<!--
	...
	export default {
		...
		methods: {
			loginPhoneNumber(e){
				...
				//console.log(e);
				uni.request({
					url: getApp().globalData.api_url+'act=connect&op=LoginOrRegisterWxx',
					method: 'POST',
					header: {
			...
			oauthLogin(provider){
				...
				uni.login({
					...
					success: (loginRes)=>{
						...
						uni.request({
							url: getApp().globalData.api_url+'act=connect&op=wxx&client=wxx&ajax=1&code='+loginRes.code,
							 success:(res) => {
