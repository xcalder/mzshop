...
<script>
	export default {
		...
		onLoad: function (option) { //option为object类型，会序列化上个页面传递的参数
			...
			this.b_id = option.b_id == undefined ? '' : option.b_id;
			this.gc_id = option.gc_id == undefined ? '' : option.gc_id;
			this.keyword = option.keyword == undefined ? '' : option.keyword;
			this.key = option.key == undefined ? '' : option.key;
			this.order = option.order == undefined ? '' : option.order;
		...
		methods:{
			getGoods(){
				...
				uni.request({
					url: getApp().globalData.api_url+'act=goods&op=goods_list&page=10&curpage='+this.curpage+'&gc_id='+this.gc_id+'&b_id='+this.b_id+'&keyword='+this.keyword+'&key='+this.key+'&order='+this.order+'&hasmore='+this.hasmore,
					 success:(res) => {
						 ...
						 for(let i in goods_list){
							 ...
							 this.goodsList.push({
								 ...
								 img: goods.goods_image_url,
								 name: goods.goods_name,
								 price: goods.goods_price,
								 slogan: '销量'+goods.goods_salenum
							 });
						 }
					 }
				});
