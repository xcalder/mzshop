<template>
	<view>
		...
		<!-- 规格-模态层弹窗 -->
		<view v-if="goodsData.spec.length > 0" class="popup spec" :class="specClass" @touchmove.stop.prevent="discard" @tap="hideSpec">
			<!-- 遮罩层 -->
			<view class="mask"></view>
			<view class="layer" @tap.stop="discard">
				<view class="content">
					...
					<view class="sp">
						<view v-for="(item,index) in goodsData.spec" :class="[item.selected?'on':'']" :key="index" @click="toGoods(item.goods_id)">{{item.spec}}</view>
					</view>
		...
		<!-- 服务-规则选择 -->
		<view class="info-box spec" v-if="goodsData.spec.length > 0">
			<!--
			...
			<view class="row" @tap="showSpec(false)">
				...
				<view class="content">
					<!--<view>选择规格：</view>-->
					<view class="sp">
						<view v-for="(item,index) in goodsData.spec" :key="index" :class="[item.selected?'on':'']">{{item.spec}}</view>
					</view>
		...
		<view class="info-box comments" id="comments">
			...
			<view v-for="(item, index) in goodsData.commentList" :key="index" class="comment" @tap="toRatings">
				<view class="user-info">
					<view class="face"><image :src="item.userface"></image></view>
...
export default {
	...
	methods: {
		getGoodsDetail(){
			uni.request({
				url: getApp().globalData.api_url+'act=goods&op=goods_detail&goods_id='+this.goods_id+'&key=',
				 success:(res) => {
					 let result = res.data.datas;
					 let goodsInfo = result.goods_info;
					 let goodsEvalList = result.goods_eval_list;
					 
					 this.swiperList = result.image_list.map((item, i) => {
						 return {
							 id: i,
							 img: item._mid
						 }
					 });
					 this.goodsData.id = goodsInfo.id;
					 this.goodsData.name = goodsInfo.goods_name;
					 this.goodsData.price = goodsInfo.goods_price;
					 
					 let spec_list = result.spec_list;
					 let spec_values = goodsInfo.spec_value;
					 let goods_spec = goodsInfo.goods_spec;
					 
					 //console.log(goods_spec)
					 for(let i in spec_values){
						 for(let d in spec_values[i]){
							 let goods_spec_select = false;
							 if(goods_spec[d] !== undefined){
								 goods_spec_select = true;
							 }
							 this.goodsData.spec.push({
								 selected: goods_spec_select,
								 goods_id: spec_list[d],
								 spec: spec_values[i][d]
							 });
						 }
					 }
					 
					 this.goodsData.commentList = goodsEvalList.map((item, i) => {
						 return {
							 number: i,
							 userface: item.member_avatar,
							 username: item.geval_frommembername,
							 content: item.geval_content
						 }
					 });
				 }
			});
		...
		getGoodsBody(){
			uni.request({
				url: getApp().globalData.api_url+'act=goods&op=goods_body&goods_id='+this.goods_id,
				 success:(res) => {
					 this.descriptionStr = res.data;
				 },
			});
		},
		//轮播图指示器
