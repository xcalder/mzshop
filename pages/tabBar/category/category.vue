	...
	<view>
        ...
        <view class="header" :style="{position:headerPosition}">
			<view class="addr">
				...
				-->
				<img class="logo" src="/static/img/home_logo.png"/>
			</view>
	...
	export default {
		data() {
			return {
				...
				categoryList:[
					/*
					{id:1,title:'品牌',banner:'/static/img/degault.png',list:[
						{name:'冰箱',	img:'1.jpg'},
						{name:'电视',	img:'2.jpg'},
						{name:'空调',	img:'3.jpg'},
						{name:'洗衣机',	img:'4.jpg'},
		...
		onLoad() {
			uni.request({
				url: getApp().globalData.api_url+'act=goods_class',
				 success:(res) => {
					let data = res.data.datas;
					let brand_list = data.brand_list.map(item => {
						return {
							id: item.brand_id,
					...
					this.categoryList.push({
						id: 0,
						title: '品牌',
						img: '/static/img/degault.png',
						list: brand_list
					});
					
					for(let i in data.class_list){
						let item = data.class_list[i];
						this.categoryList.push({
							id: item.gc_id,
							title: item.gc_name,
							img: item.image,
		...
		methods: {
			...
			toCategory(e){
				...
				uni.navigateTo({
					url: '../../goods/goods-list/goods-list?b_id='+e.id
				});
			},
			//搜索跳转
			toSearch(){
