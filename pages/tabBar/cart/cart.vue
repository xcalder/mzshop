	...
	<view>
		...
		<view class="store-list">
			...
			<view class="goods-list">
				...
				<view class="store-item" v-for="(store, index) in CartList">
					<view class="goods-list-new" v-for="(goods, index) in store.goods">
						{{ goods.goods_name }}
					</view>
				</view>
								...
								<view class="price-number">
									...
									<view class="number">
										...
										<view class="input" @tap.stop="discard">
											<input type="number" v-model="row.number" disabled="disabled" @input="sum($event,index)" />
										</view>
	...
	export default {
		...
		methods: {
			...
			getCartGoods(){
				...
				let _that = this;
				uni.request({
					url: getApp().globalData.api_url+'act=member_cart&op=cart_list',
					method: 'POST',
					header: {
					...
					data: {
					 ...
					 success:(res) => {
						 ...
						 _that.CartList = res.data.datas.cart_list;
					 }
				});
			},
			//控制左滑删除效果-begin
