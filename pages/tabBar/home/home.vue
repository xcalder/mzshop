...
export default {
	...
	onLoad() {
		uni.request({
			url: getApp().globalData.api_url+'act=index',
			 success:(res) => {
				 for(let i in res.data.datas){
					 let item = res.data.datas[i];
					 for(let key in item){
						 let item1 = item[key];
						 if(key == 'adv_list'){
							 for(let d in item1){
								 ...
								 for(let a in item2){
									 ...
									 this.swiperList.push({
										 id: x,
										 src: 'type:'+item3.type+'<||>data:'+item3.data,
										 img: item3.image
									 });
									 x++;
								 }
						 ...
						 if(key == 'goods'){
							 for(let d in item1){
								 let goodsitem = item1[d];
								 for(let i in goodsitem){
									 let goods = goodsitem[i];
									 this.productList.push({
										goods_id: goods.goods_id,
										img: goods.goods_image,
										name: goods.goods_name,
										price: goods.goods_promotion_price,
										//slogan: '1235人付款'
									 });
								 }
							 }
